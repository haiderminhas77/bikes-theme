class FacetFiltersForm extends HTMLElement {
	constructor() {
		...
		this.sideBarMode = this.classList.value.includes('side-bar-filters')
		this.facetCheckboxes = Array.from(this.getElementsByClassName('facet-checkbox'));
		...
		this.querySelectorAll('.facets__main details').forEach(detailsElement => {
			detailsElement.addEventListener('click', this.closeOtherFilters);
			detailsElement.addEventListener('keydown', this.closeOtherFilters);
		})
	...
	static renderProductCount(html) {
		...
		let iscroll = document.querySelector('infinite-scroll');
	...
	static renderFilters(html, event) {
		...
		const facetDetailsElements =
			parsedHTML.querySelectorAll('#FacetFiltersForm .js-filter, #FacetFiltersForm .js-filter');
	...
	static renderActiveFacets(html) {
		const activeFacetElementSelectors = ['.active-facets', '.active-facets-desktop'];
	...
	static renderAdditionalElements(html) {
		const mobileElementSelectors = ['.facets__open', '.facets__count', '.sorting'];
	...
	onSubmitHandler(event, externalSort) {
		event.preventDefault();

		if (this.sideBarMode) {
			const formData = new FormData(event.target.closest('form'));
			let searchParams = new URLSearchParams(formData).toString();
			const sortParamString = 'sort';
			const filterParamString = 'filter';
			...
			if (externalSort) {
				if (FacetFiltersForm.searchParamsPrev.includes(filterParamString) && !FacetFiltersForm.searchParamsPrev.includes(sortParamString)) {
					...
					searchParams = `${FacetFiltersForm.searchParamsPrev}&${searchParams}`;
			...
			if (FacetFiltersForm.searchParamsPrev.includes(sortParamString) && FacetFiltersForm.searchParamsPrev.includes(filterParamString)) {
				let sortParam = FacetFiltersForm.searchParamsPrev.split(`&${sortParamString}`)[1];
				searchParams = `${searchParams}&${sortParamString}${sortParam}`;
	...
	openActiveGroups(){
		this.facetCheckboxes.forEach((checkbox) => {
	...
	closeOtherFilters(event) {
		let clickedDetails = event.target.closest('details');
		this.closest('.facets__main').querySelectorAll('details').forEach(details => {
...
let sortButton = document.querySelector('.facets__open-filters');
let filterButton = document.querySelector('.facets__open-sort');

function openCorrectModalArea(event) {
	let areaToOpen = document.querySelector(`${event.target.classList.contains('facets__open-filters') ? '.facets__main .facets__details' : '.facets__sort-by'}`);
